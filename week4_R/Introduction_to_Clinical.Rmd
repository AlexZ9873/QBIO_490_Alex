...
```{r Exercise 1.9}
mtcars$transmission <- ifelse(mtcars$am == 0, "automatic", "manual")# write code here
...
```{r Exercise 1.11}
mtcars_copy <- mtcars[, !colnames(mtcars) %in% "am"]# write code here
```

View your new mtcars copy data frame to check that the "am" column was dropped.

```{r Exercise 1.12}
head(mtcars_copy)# write code here
```
...
```{r}
write.csv(clinic, "/home1/alexz/analysis_data/brca_clinical_data.csv", row.names=FALSE)
...
```{r Exercise 3.1}
dim(clinic)#Question 1
str(clinic$gender) #Question 2

```
```{r}
x <- as.numeric(clinic[5,"birth_days_to"]) #Question 3
x <- abs(x)
x <- x/365
print(x)
```
...
x <- as.numeric(clinic[5,"birth_days_to"]) #Question 3
x <- abs(x)
y <- as.numeric(clinic[1,"last_contact_days_to"] )
z <- ""
z <- x-y
z <- z/365
print(z)
...
```{r Exercise 4.1}
plot(clinic$birth_days_to, clinic$age_at_diagnosis)# create plot here
```
...
```{r Exercise 4.2}
boxplot(boxplot(clinic$age_at_diagnosis ~ clinic$gender))
...
```{r}
vital_status_na_mask <- clinic[!is.na(clinic$vital_status), ]
```

2) Create a mask to select for male patients
```{r}
male_mask <-clinic[clinic$gender == "male",]
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under30_over60_mask <-clinic[clinic$age_at_diagnosis < 35 | clinic$age_at_diagnosis > 60, ]
```
...
```{r}
clinic$age_at_diagnosis <- as.numeric(as.character(clinic$age_at_diagnosis))
under30_over60_mask <-clinic[clinic$age_at_diagnosis < 35 | clinic$age_at_diagnosis > 60, ]
...
```{r}
cleaned_clinic$survival_time <- ifelse(!is.na(clinic$death_days_to), clinic$death_days_to, clinic$last_contact_days_to)
```
...
The p value is 0.77, it is not statistically significant. This tells us that other factos might influence breast cancer other than age alone. 
```{r}
